		...
		try {
			...
			if (spAspects) {
				...
				for (long ind = 0; ind < nAspects; ind++) {
					...
					// gme_1088
					BOOL bEqual = FALSE;
					if ((name.Compare(_T("All")) == 0) ||
						(name.Compare(_T("Connector")) == 0) ||
						(name.Compare(_T("Modelica")) == 0) )
					{
						bEqual = TRUE;
					}
					if(name.Compare(_T("Design Space")) == 0)	// 设计空间
					{
						m_eType = DESIGNSPACE;
						bEqual = TRUE;
					}
					if(name.Compare(_T("Drivers and Monitors")) == 0)	// 测试台
					{
						m_eType = TESTBENCH;
					}
					if (!bEqual)
...
void CPartBrowserDlg::OnSelchangeAspectTab(NMHDR* /*pNMHDR*/, LRESULT* pResult)
{
	...
	if (nIndex >= 0) {
		partFrame.GetPane().ChangeAspect(nIndex);
		SendAspectChange(nIndex);
	}
	//if (s >= 0) {
